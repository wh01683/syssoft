...
cout << " OptTable " << OPTTableString[0].getName() << endl;
cout << "StringSize " << StringSize << " Location " << loc << endl;
for (int i = 0; i < getSize(); i ++) {
for (int z = loc + 1; z < 22 - loc; z ++) {
//compares the value WILL NOT READ OPTTABLESTRING CORRECTLY
cout << "Opt table i - " << OPTTable[i].getName() << " OpttableString[z] - " << OPTTableString[z].getName() << endl;
...
Value[i] = 22 + 1; //If no value matches, returns the highest possible size value. Thus it will be replaced.
}
}
}
